...
class Custom_User(AbstractUser):
    ...
    status = models.CharField(
        default="Active", max_length=20, null=True, blank=True, choices=STATUS_CHOICES
    )
    image = models.ImageField(upload_to="uploads/users/", null=True, blank=True)
    phone = models.CharField(max_length=14, null=True, blank=True)
    groups = models.ManyToManyField(Group, blank=True, related_name="custom_user_set")
    user_permissions = models.ManyToManyField(
        Permission, blank=True, related_name="custom_user_set"
    )
...
class Monitor(Product):
    ...
    display_size = models.CharField(
        max_length=20, null=True, blank=True, choices=DISPLAY_OPTIONS
    )
...
class Orders(models.Model):
    ...
    shipping_address = models.ForeignKey(
        Address, related_name="orders_shipping", on_delete=models.SET_NULL, null=True
    )
    billing_address = models.ForeignKey(
        Address, related_name="orders_billing", on_delete=models.SET_NULL, null=True
    )
    order_date = models.DateTimeField(
        default=(timezone.now() + timedelta(hours=11, minutes=00))
    )
