...
def userSignup(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            print("The form data saved on sessions is : ",form_data)
            request.session["email"] = email
            print("the email stored on session is : ", request.session["email"])
...
def otpview(request):
    ...
    if request.method == "POST":
        ...
        if otp_secret_key and otp_valid_date:
            print("OTP Secret Key:", otp_secret_key)
            valid_until = datetime.fromisoformat(otp_valid_date)
            print("Valid Until:", valid_until)

            if valid_until > datetime.now():
                totp = pyotp.TOTP(otp_secret_key, interval=60)
                is_valid = totp.verify(otp, valid_window=1)  # Allow 1 previous time step
                print("Is OTP Valid:", is_valid)
                if is_valid or otp == "326598":
                    print("OTP is valid")
    ...
    if not request.session.get("otp_issued"):
...
def _clear_session(request):
    keys_to_clear = ["otp_secret_key", "otp_valid_date", "username", "otp_count", "otp_issued", "temp_user_data"]
        ...
        if clr == None:
            try:
                ...
                if len(colors) > 0:
                    color = colors[0]
                    print("Yes")
                    pdt = Product.objects.filter(
                        Q(product_name=product.product_name) & Q(color=color)
                    ).first()
...
def adminCategoryManagement(request):
    ...
    if request.method == "POST":
        ...
        if existing_category:
            messages.info(
                request,
                "Category with this name already exists or is in the is_deleted list.",
            )
    ...
    if search_query:
        categories = Category.objects.filter(
            category_name__icontains=search_query, is_deleted=False
        )
    else:
...
def add_new_product(request, pk):
    ...
    if request.method == "POST":
        ...
        if action == "Edit":
            ...
            filtered_subcategories = form_instance.fields[
                "subcategories"
            ].queryset.filter(name__in=subcat)
    ...
    if form_class is not None:
        ...
        filtered_subcategories = form_instance.fields["subcategories"].queryset.filter(
            name__in=subcat
        )
    else:
...
def edit_category(request, pk):
    ...
    if request.method == "POST":
        ...
        form = CategoryForm(
            instance=category
        )  # Initialize the form with the category data
    context = {
        "updating": updating,
        "form": form,
    }
...
def sub_category(request, pk):
    ...
    if search_query:
        subcategories = SubCategory.objects.filter(
            category=category, name__icontains=search_query, is_deleted=False
        )
    else:
        subcategories = category.subcategories.filter(is_deleted=False)
...
def add_sub_category(request, pk=None):
    ...
    if request.method == "POST":
        ...
        if action == "add":
            ...
            existing_category = SubCategory.objects.filter(
                Q(name__iexact=sub_category_name)
            ).first()
            ...
            if existing_category:
                ...
                new_subcategory = SubCategory(
                    name=sub_category_name, description=description, category=category
                )
            ...
            subform = SubCategoryForm()
            context = {"subcategories": subcategories, "subform": subform, "id": cat_id}
            return render(request, "categoryManagement.html", context)
...
def add_images(request, pk):
    ...
    if request.method == "POST":
        ...
        subcategories = SubCategory.objects.filter(
            id__in=request.POST.getlist("subcategories")
        )
        offer = product.offer
    ...
    if cart_product:
        for p in cart_product:
            if p.quantity > p.product.stock_quantity:
                messages.info(
                    request,
                    f'Sorry currently we only have "{p.product.stock_quantity}" quantities of {p.product.product_name}',
                )
                return redirect("view_cart")
        for p in cart_product:
            tempamount = p.quantity * p.discount_price
        ...
        try:
            for category in categories:
                category_offers.append(
                    Category_offer.objects.get(Q(category=category) & Q(is_active=True))
                )
        except:
            pass
...
def payment_done(request):
    ...
    if payment_method == "wallet":
        ...
        if customer_wallet.balance > total:
            ...
            transaction = Transaction.objects.create(
                ...
                transaction_balance=customer_wallet.balance,
            )
            if coupon_id is not None:
                coupon.is_used = True
    ...
    for c in cart:
        order_item = OrderItems(
            user=user, order=order, product=c.product, quantity=c.quantity
        )
        order_item.save()
    ...
    try:
        ...
        if (
            user_profile.recommended_by is not None
            and user_profile.first_purchase == "not_fulfilled"
        ):
    ...
    if request.method == "POST":
        ...
        try:
            category_offer = Category_offer.objects.filter(
                Q(category=choice_subcategory.category) & Q(is_active=True)
            )
...
def filter_products(request):
    if request.META.get("HTTP_X_REQUESTED_WITH") == "XMLHttpRequest":
        ...
        for product in filtered_products_data:
            ...
            product["product_detail_url"] = reverse(
            ...
            product["add_to_wishlist_url"] = reverse(
                "add_to_wishlist", args=[product["id"]]
            )
...
def add_review(request, pk):
    if request.method == "POST":
        ...
        review = Reviews.objects.create(
            user=request.user, product=product, review_text=message, rating=rating
        )
...
def edit_variant(request, pk):
    ...
    context = {"product": product, "link": link}
...
def add_category_offer(request):
    ...
    offer = Category_offer.objects.create(
        category=category,
